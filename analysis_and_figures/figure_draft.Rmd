...
f1a <- rec %>%
  ...
  geom_label(
    data = mean_recovery, 
    inherit.aes = F,
    aes(label = paste0(round(as.numeric(mean_recovery), digits = 1), "%", " ±", round(sd_recovery, digits = 1)), fill = Method),
    x = Inf, 
    y = Inf,
    vjust = 1.8,
    hjust = 1.1,
...
```{r Pub_Figure_1, fig.height=9, fig.width=17}
...
#ggsave(plot = figure3, filename = "./publication_ready_figure/figure3.png", height = 4, width = 12)
```
...
figure4 <- f4
#ggsave(plot = figure4, filename = "./publication_ready_figure/figure4.png", height = 4, width = 12)
```
...
```{r Inhibition levels}
inhib <- read.delim(
  "/home/mthomt69/personal_folder_BACKUP/comparative_ww_virome_analysis_manuscript/analysis_and_figures/data_files/WWDv_001e_inhibition.csv",
                    sep = ",", header = T) %>%
  mutate(
    Method = case_when(
...
s3a <- ggplot(rec_trans, aes(x = expected_conc, y = Concentration)) + 
  ...
  labs(
    title = "Recovery from expected quantification",
...
#(s3a + s3b) + plot_layout(widths = c(3,1)) + plot_annotation(tag_levels = "A") #+ canvas(height=4, width = 15)
s3a
```
...
### S6: Viral charactersitcs model output

- For figure generation code see `model_viral_characteristics.R`


### S7: Genus level distribution of bias
```{r Suppl_Fig_7}
...
s8 <- (p | v | n) / (u | m | un) + plot_layout(guides = "collect", axes = "collect_x", axis_titles = "collect_y")
s8
#ggsave(s8, filename = "analysis_and_figures/publication_ready_figure/supplement_figures/n_missing_taxa.png", height = 4, width = 14, units = "in")
```
