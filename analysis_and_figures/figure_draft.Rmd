...
author: "Matthew Thornton, Gabriela Eder, Fabian Amman, Anastasiia Pantielieieva, Julia Vierheilig, Andreas Bergthaler"
date: "`r Sys.Date()`"
output:
  html_document: 
    code_folding: hide
...
plot_pcoa <- function(matrix, title, df){
  ...
  p <- plot_pts %>%
    ...
    geom_point(size = 2.5, aes(fill = Method), shape = 21, alpha = 0.8, show.legend = F)+ 
    theme_bw() +
...
join_dat <- seq_dat_updated_wwtp %>% 
  dplyr::select(WWTP, Method, Replicate, "accession" = Accession, strain, genus, species, family, bases_covered_10x)
...
-   Kraken data used for Fig 2b overview of sample composition. Otherwise all analysis is based on `seq_data`
...
```{r Fig_1A_recovery, fig.width=12, fig.height=5}
# Calculate reference value for 'perfect' volumetric recovery
dtn <- dpcr %>% 
  filter(Method == "UNC") %>%
  group_by(Target) %>%
  mutate(
    # Adjust for unconcentrated dilution factor (500uL -> 100uL)
    # Since the concentration is in 100uL the conc in the original 500uL needs to be divided by 5
    Concentration = Concentration / (500 / 100), 
    mean_ref_quant = mean(Concentration)
    ) %>%
  ungroup() %>%
...
rec <- dpcr2 %>%
  ...
  mutate(
    ...
    recovery = Concentration / expected_conc 
    )
...
f1a <- rec %>%
  ...
  geom_text(
    aes(label = paste0(round(mean_recovery, digits = 2)), y = mean_recovery + sd_recovery), 
    vjust = -0.5, 
    size = 2, 
    ) +
  geom_label(
    data = mean_recovery, 
    inherit.aes = F,
    aes(label = paste0(round(as.numeric(mean_recovery), digits = 2), " ±", round(sd_recovery, digits = 2)), fill = Method),
    x = Inf, 
    y = Inf,
    vjust = 1.8,
    hjust = 1.1,
  ...
  labs(
    title = "Concentration performance per target",
    y = "[Measured] / [Expected]",
    x = ""
  ) 
...
```{r Pub_Figure_1, fig.height=4, fig.width=14}
...
ggsave(filename = "./publication_ready_figure/figure1.png", figure1, height = 4, width = 14, dpi = 300)
```

# Figure 2: Evaluating sequencing performance per method

**Figure objective**: Introduce the results of the sequencing and how each method performed by various metrics include % reads on target, species richness, reproducibility and depth of viral detection , community similarity, and fold enrichment compared to mean unconcentrated read count.

## Figure 2A: Percent read composition (kraken2 data)

```{r 2A kraken classification, fig.height=4, fig.width=10}
...
f2a <- ggplot(domain_plot, aes(x = Sample, y = Percent_fragments, fill = Tax_name, alpha = Tax_name)) +
  geom_bar(stat = "identity", color = "white", width = 1, position = "fill", alpha = 0.8) +
  theme_bw() + 
  ...
  scale_fill_manual(values = c("grey70", "#E3D36B", "#7A8FA3", "#72BBA8", "sienna2")) +
...
```{r Figure 2B Heatmap, fig.height=20, fig.width=12}
heat_dat <- wwdv_002 %>%
...
f2b <- ggplot(heat_dat, aes(x = Sample, y = reorder(species, Salmon_read_number), fill = Salmon_read_number, group = Method)) +
  ...
  scale_fill_viridis(discrete = F, trans = "log10", name = "A) Virus abundance \nlog(Read counts)") +
  facet_grid(~Method, scales = "free_x") +
  theme(
...
```{r Figure 2C SRR, fig.height=4, fig.width=6}
...
f2d <- hve + theme_minimal() + guides(fill = guide_legend(override.aes = list(alpha = 0.8))) + labs(fill = "C,D,E) Method")
...
dunn_res <- dunn_res %>% 
  mutate(
    group1 = ifelse(group1 == "PEG", group2, group1),
    group2 = ifelse(group2 != "PEG", "PEG", group2),
  ) 

dunn_res$group1 <- factor(dunn_res$group1, levels = c("MEM", "UF", "NT", "VDC"))
...
f2c <- f2c + guides(fill = "none")
f2e <- f2e + guides(fill = "none")

figure2 <- f2b + (f2a / (f2c | f2d) / (f2e + theme_minimal()) + plot_layout(heights = c(1,1,2))) + 
...
figure2 <- figure2 + plot_layout(design = layout) + plot_annotation(tag_levels = "A")
figure2

ggsave(filename = "./publication_ready_figure/figure2.png", figure2, height = 10, width = 12)
```

# Figure 3: Modelling viral taxonomy and enrichment efficiency

**Figure objective**: Investigate methods consistency to enrich viruses across varied characteristics (ie. taxonomic grouping or morphological features) by modelling the enrichment rate as a response variable to an interaction of the method and characteristics. Compute mariginal effects at the level of the method/charactertics.

## Figure 3: Modelling methods with taxonomy interaction
- Output: controlled FWER with holm method on all p-values

```{r Fig 3 Prep model data, warning=FALSE}
# Prep data
vir <- wwdv_002 %>%
  group_by(Method, Replicate, strain, species, genus, family) %>%
  summarize(Salmon_read_number = sum(Salmon_read_number)) %>%
  ungroup() %>%
  dplyr::select(Method, Replicate, strain, species, genus, family, Salmon_read_number) %>%
  group_by(Method, strain) %>%
  mutate(mean_reads = mean(Salmon_read_number)) %>% 
  mutate(
...
ref <- vir %>% 
  ...
  dplyr::select(strain, Replicate, species, genus, family, ref_reads, ref_mean_reads)

# Divide each sample replicate by the mean of the reference
mod2_dat <- vir %>% 
  dplyr::left_join(ref %>% ungroup() %>% dplyr::select(-Method)) %>% 
  dplyr::select(-mean_reads) %>%
  mutate(fold_enrichment_reads = Salmon_read_number / ref_reads) %>% 
...
sig <- out_fam %>%
  dplyr::select(term, family, p.value) %>%
...
error_size <- jitter_dat %>% 
  ...
  left_join(jitter_dat %>% ungroup() %>% dplyr::select(Method, contrast)) %>%
...
```{r Fig 3 Taxonomy, fig.height=4, fig.width=12}
# Sign colors
sig_colors <- c(
  "ns" = "grey",
  "(+)Sig***" = "#F45C10",
...
# Set plot order
plot_f$contrast <- factor(plot_f$contrast, levels = c("MEM", "UF", "NT", "VDC", "PEG"))
jitter_dat$contrast <- factor(jitter_dat$contrast, levels = c("MEM", "UF", "NT", "VDC", "PEG"))
...
f3 + ggview::canvas(height = 7, width = 9, units = "in")
```

## Publication: Figure 3  
```{r Pub Figure 3, fig.height=4, fig.width=12}
figure3 <- f3
 
ggsave(plot = figure3, filename = "./publication_ready_figure/figure3.png", height = 7, width = 9, dpi = 300)
```
...
for (i in 1:nrow(method_frame)) {
  ...
  df1 <- kwd_002 %>% 
    ungroup() %>%
    dplyr::select(WWTP, Method, Replicate, accession, pi) %>%
  ...
  sub <- df1 %>%
    dplyr::select(WWTP, Method, Replicate, accession, pi, comparison) %>%
    group_by(accession) %>% 
    filter(n() == 6) %>%
...
res <- out_df %>%
  dplyr::select(comparison, pvalue) %>%
...
res$comparison <- factor(res$comparison, levels = c("PEG-UNC", "PEG-MEM", "PEG-UF", "PEG-NT", "PEG-VDC"))
out_df$comparison <- factor(out_df$comparison , levels = c("PEG-UNC", "PEG-MEM", "PEG-UF", "PEG-NT", "PEG-VDC"))
out_df$Method <- factor(out_df$Method , levels = c("UNC", "MEM", "UF", "NT", "VDC", "PEG"))

```

```{r Fig 4 plot, fig.height=5, fig.width=12, warning=FALSE}
percentiles <- out_df %>% 
  group_by(Method) %>% 
...
```{r Pub Figure 4, fig.height=4, fig.width=12}
figure4 <- f4
ggsave(plot = figure4, filename = "./publication_ready_figure/figure4.png", height = 5, width = 12)
```
...
```{r Prep 5A model data, warning=FALSE}
...
```{r Fig 5A taxonomic_model, height=5, width=6}
bias_colors <- c(
  "ns" = "grey",
  "PEGbias***" = "#F45C10",
  "PEGbias**" = "#FB8A30",
...
## Figure 5B: Reproducibility across WWTP

```{r Figure 5B, height=8, width=10, message=F, warning=F}
...
```{r Pub Figure 5, warning=F, fig.width=15, fig.height=6}
layout <- "
AABBEEEE
CCDDEEEE
"
...
ggsave(plot = figure5, filename = "./publication_ready_figure/figure5.png", height = 5, width = 11)
```
...
```{r Suppl_Fig_1, fig.height=6, fig.width=6}
knitr::include_graphics("./publication_ready_figure/supplemental_figures/supp_fig1.png")
```
...
```{r Suppl_Fig_2, fig.height=6, fig.width=9, eval=T}
mean_quants <- dpcr_cleaned %>%
  filter(!Method %in% c("DE", "UNC", "VDCP")) %>%
  filter(!Target %in% c("Astrovirus", "RSV", "HPV-E1")) %>%
  mutate(Method = ifelse(Method == "UNC_Sp", "UNC", Method)) %>%
  group_by(Method, Target) %>%
  summarize(mean_quant = mean(Concentration),
            sd_quant = sd(Concentration))

mean_quants$Method <- factor(mean_quants$Method, levels = c("UNC", "MEM", "UF", "NT", "VDC", "PEG"))

# plot total sum of all mean virus quants per method
sf2a <- ggplot(mean_quants, aes(x = Method, y = mean_quant, fill = Method)) +
  theme_bw() +
  geom_bar(
    stat = "identity", 
    alpha = 0.6, 
...
```{r Background levels of spike ins, fig.height=12, fig.width=9}
# Background levels of spike-ins in WW
file <- "./data_files/dPCR_no-spike-controls.csv"
bg <- read.delim(file, sep = ",", header = T) %>% filter(Method != "VDCP")

# Calculate as a percentage of total signal
to_join <- dpcr_cleaned %>% 
  filter(Viral_type == "Spike-in") %>% 
  rename("Conc_total" = "Concentration") %>% 
  dplyr::select(Method, Replicate, Target, Conc_total) %>%
  group_by(Method, Target) %>%
  summarize(mean_spike_conc = mean(Conc_total))

bg2 <- bg %>% 
  left_join(to_join) %>%
  mutate(bg_percent_total = Concentration / mean_spike_conc * 100)

# Create plots
bg$Method <- factor(bg$Method, levels = c("MEM", "UF", "NT", "VDC", "PEG"))
sf2b <- ggplot(bg, aes(x = Method, y = Concentration, fill = Method)) +
  geom_bar(stat = "identity", color = "black", alpha = 0.6) +
  facet_wrap(~Target, nrow = 1) +
  scale_fill_manual(values = pal) + canvas(width = 6, height = 3) +
  theme(
    axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
    legend.position = "none"
    ) +
  labs(
    title = "Background concentrations of spike-ins in WW",
    y = "Concentration [ng/uL]"
  )
  
bg2$Method <- factor(bg2$Method, levels = c("MEM", "UF", "NT", "VDC", "PEG"))
sf2c <- ggplot(bg2, aes(x = Method, y = bg_percent_total, fill = Method)) +
  geom_bar(stat = "identity", color = "black", alpha = 0.6) +
  facet_wrap(~Target, nrow = 1) +
  scale_fill_manual(values = pal) + canvas(width = 6, height = 3) +
  theme(
  ...
  labs(
    title = "Background as a percentage of total signal",
    y = "% total signal"
  )
...
```{r Inhibition levels}
inhib <- read.delim(
  "./data_files/WWDv_001e_inhibition.csv",
                    sep = ",", header = T) %>%
  mutate(
    Method = case_when(
      substring(sample, 1, 3) == "UNC" ~ "UNC",
      substring(sample, 1, 2) == "NT" ~ "NT",
      substring(sample, 1, 2) == "UF" ~ "UF",
      substring(sample, 1, 3) == "MEM" ~ "MEM",
      substring(sample, 1, 3) == "PEG" ~ "PEG",
      substring(sample, 1, 3) == "VDC" ~ "VDC"
    ),
    Replicate = case_when(
      endsWith(sample, "1") ~ "R1",
      endsWith(sample, "2") ~ "R2",
      endsWith(sample, "3") ~ "R3"
    )
  )

# Join recoveries per target per replicate with inhibition data
p <- rec %>% left_join(inhib, by = c("Method", "Replicate")) %>%
  group_by(Method, Replicate, inhibition_factor) %>%
  summarize(mean_recovery = mean(recovery))

m <- lm(mean_recovery ~ inhibition_factor, data = p)
summary(m)

sf2d <- ggplot(p, aes(x = inhibition_factor, y = mean_recovery)) +
  ...
  annotate(
    ...
    y = Inf,
    vjust = 1.25,
    label = expression(paste(R^2," = 0.359; pval = 0.011"))
    ) +
  geom_point(
  ...
  theme_bw() +
  labs(
    title = "Inhibition vs. performance",
    x = "Inhibition Factor",
    y = "Mean performance"
    )
```

```{r SF2 plot, fig.height=11, fig.width=11}
sf2 <- sf2a / ((sf2b / sf2c) | sf2d) + 
  plot_layout(heights = c(1.5,1)) + 
  plot_annotation(tag_levels = "A") 

ggsave("./publication_ready_figure/supplemental_figures/suppl_fig_2.png", sf2, height = 11, width = 11, dpi = 300)
```


### S3: Recovery scatterplot
-   Plotting the "recovery" of each viral target against the quantifications of the unconcentrated sample adjusted for volume (80x factor).

```{r Suppl_Fig_3, fig.height=4, fig.width=12}
# Recovery scatterplot
rec_trans <- rec %>% mutate(expected_conc = log10(expected_conc), Concentration = log10(Concentration))
res_m <- data.table(Method = character(), slope = numeric(), std_error = numeric())
for (s in unique(rec$Method)){
  dtm <- rec_trans %>% filter(Method == s) %>% mutate(Concentration = Concentration, expected_conc = expected_conc)
  formula.minimal <- as.formula(paste("Concentration", "~", "expected_conc"))
  model.minimal <- lm(formula.minimal, data = dtm)
  res_m <- rbind(res_m, data.table(
    Method = s, 
    slope = summary(model.minimal)$coef[2],  
    std_error = summary(model.minimal)$coef[4]
      )
    )
}

rec_trans$Method <- factor(rec_trans$Method, levels = c("MEM", "UF", "NT", "VDC", "PEG"))
res_m$Method <- factor(res_m$Method, levels = c("MEM", "UF", "NT", "VDC", "PEG"))

s3a <- ggplot(rec_trans, aes(x = expected_conc, y = Concentration)) + 
  geom_point(
    aes(fill = Target, color = Target), 
    size = 2.5,
    alpha = 0.75,
    shape = 21
  ) + 
  ...
  geom_text_repel(
    data = rec_trans %>% 
      filter(Replicate == "R1") %>% 
      mutate(Target = gsub("Influenza", "Infl.", Target)) %>% 
      mutate(Target = gsub("SARS-CoV-2", "SC2", Target))  %>% 
      mutate(Target = gsub("virus", "v.", Target)) %>% 
      dplyr::select(Method, Target, Concentration, expected_conc) %>% 
      distinct(), 
    aes(label = Target), 
    size = 2.3, 
    alpha = .5
  ...
  geom_text(
    data = res_m,
    aes(label = paste0("Slope: ", round(slope, digits = 2), "\nStd.err: ", round(std_error, digits = 3))),
    x = 0.01,
    y = 5.5,
    alpha = 1,
    size = 2.75,
  ...
  labs(
    title = "Quantification comparison to benchmark",
    x = "Log10 Expected quantification [cp/uL]",
    y = "Log10 Observed quantification [cp/uL]",
    fill = "Target"
  ) +
...
# Coef of variation
mean_quants <- dpcr %>%
  filter(!Method %in% c("DE", NA)) %>%
  group_by(Target, Method) %>%
  summarise(
    mean_quant = mean(Concentration),
    mean_quant_adj = mean(Concentration_adj),
    sd_quant = sd(Concentration),
    sd_quant_adj = sd(Concentration_adj),
    cv = sd_quant_adj/mean_quant_adj
    ) %>%
...
s3b <- ggplot(mean_quants %>% filter(Target != "16S"), aes(x = Method, y = cv)) +
  geom_boxplot(
    fill = "grey",
    alpha = 0.5, 
    outliers = F, 
...
sf3 <- (s3a + s3b) + plot_layout(widths = c(3,1)) + plot_annotation(tag_levels = "A") #+ canvas(height=4, width = 15)
ggsave("./publication_ready_figure/supplemental_figures/suppl_fig_3.png", sf3, height = 4, width = 14, dpi = 300)
```

### S4: Read fate plot
```{r Suppl_Fig_4, fig.height=4, fig.width=7}
readcountdata <- fread("./data_files/mappings_stats_FA.csv")
...
ggsave(plot = fs, filename = "./publication_ready_figure/supplemental_figures/suppl_fig_4.png", height = 4, width = 7, dpi = 300)
```


### S5: Compare predicitive power of dPCR to seq data

```{r Suppl_Fig_5, fig.height=5, fig.width=12}
comp <- seq_dat %>% 
  filter(
    Experiment == "WWDv_002",
...
plot_comp <- dpcr2 %>% 
  ...
  left_join(comp %>% dplyr::select(Method, Replicate, species, relative_abund_seq)) %>%
...
ggsave(
  plot = last_plot(), 
  filename = "./publication_ready_figure/supplemental_figures/suppl_fig_5.png", 
...
### S6: Heatmap with species names
```{r suppl_fig_6_heatmap}
sf6 <- ggplot(heat_dat, aes(x = Sample, y = reorder(species, Salmon_read_number), fill = Salmon_read_number, group = Method)) +
  geom_tile(color = "white") +
  geom_vline(color = "white", xintercept = c(3.5, 6.5, 9.5, 12.5, 15.5), linewidth = 1) +
  scale_fill_viridis(discrete = F, trans = "log10", name = "A) Virus abundance \nlog(Read counts)") +
  facet_grid(~Method, scales = "free_x") +
...
ggsave("./publication_ready_figure/supplemental_figures/suppl_fig_6.png", height = 20, width = 12, dpi = 300)
```


### S7: Viral charactersitcs model output

- For figure generation code see `model_viral_characteristics.R`


### S8: Genus level distribution of bias
```{r Suppl_Fig_7}

```


### S8: Number of missing taxa relative to each method
```{r Suppl_Fig_8, fig.height=8, fig.width=10}
plot_missing_taxa <- function(df, ref_level) {
  ...
  result <- df %>%
    #filter(Method != {{ref_level}}) %>%
    group_by(Method) %>%
    dplyr::select(Method, strain, species, genus, family, class, order, phylum) %>%
    distinct() %>%
    summarize(
...
s9 <- (p | v | n) / (u | m | un) + plot_layout(guides = "collect", axes = "collect_x", axis_titles = "collect_y")

ggsave(
  s9, 
  filename = "./publication_ready_figure/supplemental_figures/suppl_fig_9.png", 
...
properties <- rio::import("./data_files/WWDv_001e_viral_properties.csv")
kable(properties)
```

### Table S: Concentrations of spike-in viruses
```{r spike-in concentration table}
# 1:10 diluted DE samples were already multiplied by 10 in file_processing_script
de <- dpcr %>% 
  filter(Method == "DE",  Viral_type == "Spike-in") %>%
  dplyr::select(-Sample, -Concentration_adj)

# Set variables
dpcr_rxn_dilution_factor <- 40/2 # 2uL input in 40uL total reaction volume
initial_volume <- 1500 # uL
extract_volume <- 90   # uL
ww_volume <- 600       # mL

# Calculate theoretical concentration in 600mL WW
## Since the replicates are actually sub-divisions of 500ul each of the same 1500L stock we add the n_copies from each replicate

# Get copy numbers of each target in the extract
de_conc_extract <- de %>%
  rowwise() %>%
  mutate(
    extract_concentration = Concentration * dpcr_rxn_dilution_factor,
    n_cps_extract = extract_concentration * extract_volume
    )

# Sum total copies for each target
total_cps_per_target <- de_conc_extract %>%
  group_by(Target) %>%
  summarize(total_cps = sum(n_cps_extract))

# Calculate concentration in 600mL WW
spike_in_concs <- total_cps_per_target %>%
  mutate(
    spiked_conc_per_uL = total_cps / (ww_volume * 1000),
    spiked_conc_per_mL = total_cps / ww_volume
  )

kable(
  spike_in_concs, 
  col.names = c("Target", "Total Genome Copies", "Spiked concentration [gc/uL]", "Spiked concentration [gc/mL]"), 
  caption = "Quantification of spiked-in viral targets"
  )
```
...
## Modelling data: Facet by method

```{r Model with method facet, height=5, width=15, eval=F}
...
```{r Raw viral characterisitcs, height=12, width=12, eval=F}
